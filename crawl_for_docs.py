...
ollama_client = AsyncClient()  # Specify the correct base URL
...
async def get_title_and_summary(chunk: str, url: str) -> Dict[str, str]:
    ...
    try:
        ...
        response = await ollama_client.chat(
            ...
            format=ChunkSummary.model_json_schema()
        )
...
def get_the_docs_urls(sitemap_url: str) -> List[str]:
    ...
    try:
        ...
        # Extract all URLs and nested sitemaps from the sitemap
        namespace = {'ns': 'http://www.sitemaps.org/schemas/sitemap/0.9'}
        for sitemap in root.findall('.//ns:sitemap', namespace):
            loc = sitemap.find('ns:loc', namespace).text
            urls.extend(get_the_docs_urls(loc))
